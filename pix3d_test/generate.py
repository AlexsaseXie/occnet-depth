...
for it, data in enumerate(tqdm(test_loader)):
    ...
    if cur_image_category != 'n/a':
        mesh_dir = os.path.join(mesh_dir, str(cur_image_category))
        pointcloud_dir = os.path.join(pointcloud_dir, str(cur_image_category))
        in_dir = os.path.join(in_dir, str(cur_image_category))

        folder_name = str(cur_image_category)
    ...
    if cfg['generation']['copy_groundtruth']:
        modelpath = os.path.join(
            dataset.pix3d_root, cur_image_info['model']
        )
    ...
    if generate_mesh:
        ...
        # Write output
        mesh_out_file = os.path.join(mesh_dir, '%s.off' % cur_image_name)
    ...
    if cfg['generation']['copy_input']:
        # Save inputs
        if input_type == 'img':
            inputs_path = os.path.join(in_dir, '%s.jpg' % cur_image_name)
            inputs = data['inputs'].squeeze(0).cpu()
            visualize_data(inputs, 'img', inputs_path)
            out_file_dict['in'] = inputs_path
        elif input_type == 'voxels':
            inputs_path = os.path.join(in_dir, '%s.off' % cur_image_name)
            inputs = data['inputs'].squeeze(0).cpu()
            voxel_mesh = VoxelGrid(inputs).to_mesh()
            voxel_mesh.export(inputs_path)
            out_file_dict['in'] = inputs_path
        elif input_type == 'pointcloud':
            inputs_path = os.path.join(in_dir, '%s.ply' % cur_image_name)
