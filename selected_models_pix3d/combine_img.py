...
def transpose_main(sticker_list, out_dir='final_stickers', vertical_sticker_count=13, shuffle=False):
    if not os.path.exists(sticker_list):
        return
    ...
    for i in tqdm(range(total_sticker_num)):
        if i != total_sticker_num - 1:
            ...
            cur_vertical_sticker_num = len(model_info_list) - vertical_sticker_count * i 
        new_size = (RENDERING_RESOLUTION * cur_vertical_sticker_num, RENDERING_RESOLUTION * len(FILES))
        new_image = Image.new('RGBA', new_size)
        ...
        for k in range(cur_vertical_sticker_num):
            ...
            model_root = os.path.join(SELECTED_MODEL_ROOT, class_id, modelname)
            ...
            for idx, f in enumerate(FILES):
                ...
                new_image.paste(image, (k * RENDERING_RESOLUTION, idx * RENDERING_RESOLUTION))
...
if __name__ == '__main__':
    ...
    if args.vertical:
        vertical_sticker_file = os.path.join(SELECTED_MODEL_ROOT, args.vertical_sticker_file)
        vertical_main(
            vertical_sticker_file, 
