...
for it, data in enumerate(tqdm(test_loader)):
    ...
    out_dict = checkpoint_io.load(cfg['test']['model_file'])
    ...
    t0 = time.time()
    meshes = generator.generate_mesh(data, out_dict, separate=True)
    time_dict['mesh'] = time.time() - t0

    # Write output
    for idx, mesh in enumerate(meshes):
        mesh_out_file = os.path.join(generation_dir, '%d.off' % idx )
