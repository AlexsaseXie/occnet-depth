...
parser.add_argument('--save_folder', type=str, default='spsr_fps30000')
parser.add_argument('--dataset_root', type=str, default='/home3/xieyunwei/ShapeNet.SAL.clean/')
parser.add_argument('--list_file', type=str, default='train_instance.lst')
...
def get_list():
    ...
    for c in classes:
        ...
        lst_path = os.path.join(args.dataset_root, c, args.list_file)
        ...
        with open(lst_path, 'r') as f:
            ...
            modelnames = [ tmp.split(' ')[1] for tmp in modelnames]

        print(modelnames)
        print('class %s modelname count: %d' % (c, len(modelnames)))
        for modelname in modelnames:
            target_file = os.path.join(class_root, modelname, 'pointcloud_fps_N30000.npz')
...
def process_file(pc_file, output_folder=None):
    model_root = os.path.dirname(pc_file)
